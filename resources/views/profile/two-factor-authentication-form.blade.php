    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                <div class="mt-4 p-2 inline-block bg-white shadow-lg rounded-lg">
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        <x-label for="code" value="{{ __('Código') }}" class="font-semibold text-gray-700" />

                        <x-input id="code" type="text" name="code" class="block mt-1 w-1/2 border-gray-300 rounded-md shadow-sm" inputmode="numeric" autofocus autocomplete="one-time-code"
                            wire:model="code"
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Guarda estos códigos de recuperación en un administrador de contraseñas seguro. Pueden ser usados para recuperar el acceso a tu cuenta si pierdes tu dispositivo de autenticación de dos factores.') }}
                    </p>
                </div>

                <div class="grid gap-1 max-w-xl mt-4 px-4 py-4 font-mono text-sm bg-gray-100 rounded-lg shadow-lg">
